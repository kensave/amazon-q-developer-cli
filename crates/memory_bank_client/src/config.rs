//! Configuration management for the memory bank client.
//!
...
/// Main configuration structure for the memory bank client.
...
pub fn init_config(base_dir: &Path) -> std::io::Result<()> {
    let config_path = base_dir.join("memory_config.json");
...
pub fn get_config() -> &'static MemoryConfig {
    CONFIG.get().expect("Memory configuration not initialized")
}
...
fn load_or_create_config(config_path: &Path) -> std::io::Result<MemoryConfig> {
...
///
/// A Result indicating success or failure
fn save_config(config: &MemoryConfig, config_path: &Path) -> std::io::Result<()> {
...
///
/// A Result indicating success or failure
pub fn update_config(base_dir: &Path, new_config: MemoryConfig) -> std::io::Result<()> {
    let config_path = base_dir.join("memory_config.json");
...
mod tests {
    ...
    fn test_load_or_create_config() {
        ...
        let config_path = temp_dir.path().join("memory_config.json");
    ...
    fn test_update_config() {
        ...
        let loaded_config: MemoryConfig = serde_json::from_str(&content).unwrap();
