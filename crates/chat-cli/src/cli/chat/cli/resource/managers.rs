    ...
    async fn execute(&mut self, operation: ResourceOperation) -> Result<ResourceData> {
        match operation {
            ...
            ResourceOperation::Update { .. } => {
                Ok(ResourceData::Success("Update not supported for pinned resources".to_string()))
            }
            ResourceOperation::Show { expand: _ } => {
                let mut profile_context_files = std::collections::HashSet::<(String, String, bool)>::new();
                let (agent_owned_list, session_owned_list) = self.manager
                    .paths
                    .iter()
                ...
                for path in &agent_owned_list {
                    let context_files = self.manager
                        .get_context_files_by_path(self.os, path.get_path_as_str())
                        .await
                        .unwrap_or_default();
    ...
    async fn execute(&mut self, operation: ResourceOperation) -> Result<ResourceData> {
        ...
        match operation {
            ResourceOperation::Add { name, value, include_patterns, exclude_patterns, index_type } => {
                ...
                let id = store.add(&name, &value, options).await
            ...
            ResourceOperation::Update { path } => {
                let message = store.update_by_path(&path).await
                    .map_err(|e| eyre::eyre!("{}", e))?;
